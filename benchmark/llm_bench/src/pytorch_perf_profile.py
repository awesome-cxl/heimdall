            ...
            if index == end_index:
                if measurement_process is not None:
                    ...
                    timer_thread = threading.Thread(
                        target=terminate_after_timeout, args=(measurement_process, timeout)
                    )
                    timer_thread.start()
                    timer_thread.join()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Performance profiling script for PyTorch"
    )
    parser.add_argument('cpu_bind', type=int, nargs='?', default=-1,
                        help="CPU binding number (default: -1, meaning no CPU binding)")
    parser.add_argument('mem_bind', type=int, nargs='?', default=0,
