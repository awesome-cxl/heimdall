...
def install_deps():
    with h_utils_path.chdir(h_utils_path.get_workspace_path() / "benchmark/lockfree_bench"):
        ...
        run_command("mkdir -p libs")
        run_command("mkdir -p results")
        
        # install python packages using apt
        run_command("sudo apt install -y python3-numpy python3-pandas")

        # install boost using apt
        # install libcds using apt
        run_command("sudo apt install -y libboost-all-dev libcds-dev")

        #install cxxopt from github
        run_command("git clone https://github.com/jarro2783/cxxopts.git libs/cxxopt")

        # install folly from github
        run_command("sudo apt install -y libgoogle-glog-dev")
        run_command("git clone https://github.com/facebook/folly.git downloads/folly")
        run_command("mkdir -p libs/folly")
        run_command("cd downloads/folly && "
                    "git checkout v2025.02.10.00")
        run_command("cd downloads/folly && "
                    "sudo ./build/fbcode_builder/getdeps.py install-system-deps --recursive")
        run_command("cd downloads/folly && "
                    "./build/fbcode_builder/getdeps.py "
                    "--num-jobs=$(nproc) --install-prefix=$(realpath ../../libs/folly) build")
        
        # install junction from github
        run_command("git clone https://github.com/preshing/junction.git downloads/junction")
        run_command("git clone https://github.com/preshing/turf.git downloads/turf")
        run_command("mkdir -p libs/junction && "
                    "cd downloads/junction && "
...
def run(machine: str, timestamp: str, results: map):
    with h_utils_path.chdir(h_utils_path.get_workspace_path() / "benchmark/lockfree_bench"):
        ...
        run_command("echo 0 | sudo tee /proc/sys/kernel/numa_balancing")
        ...
        for file_path in glob.glob("/sys/devices/system/cpu/cpu*/cpufreq/scaling_governor"):
            run_command(f"echo perofrmance | tee {file_path}")
            
        loop_rounds = 1000000

        ds_configs = {
            "queue": ["boost_spsc_queue",
                    "boost_mpmc_queue"],
            "map": ["folly_atomichashmap_map",
        ...
        #   machine_name: {
        #       numa_config_name: [
        #           setter_core, getter_core, setter_numa_node, getter_numa_node, ds_numa_node
        #       ]
        #   }
        # }
        numa_configs = {
                    ...
                    for numa_config_name, numa_config in numa_configs[machine].items():
                        ...
                        for ds_size_mb in ds_size_mbs:
                            ...
                            while (time_val_get < 10):
                                if (ds_size_mb <= 64):
                                    stdout, _, ret_code = run_command_with_timeout(cmd, 10)
                                elif (ds_size_mb <= 256):
                                    stdout, _, ret_code = run_command_with_timeout(cmd, 60)
                                else:
                                    stdout, _, ret_code = run_command_with_timeout(cmd, 1800)
                                
                                if (ret_code != -1 and ret_code != -2):
                                    ...
                                    f.write(f"Config: {ds} {ds_type} {numa_config_name} {ds_size_mb}MB {time_val_get + 1}\n"
                                            f"Command: {cmd}\n"
                                            f"Output: {stdout}\n")
                                else:
                                    print(f"Error {ret_code}")
                                    second_try_times += 1
                                    if (second_try_times > 20):
                                        break
                                    else:
                                        f.write(f"Config: {ds} {ds_type} {numa_config_name} {ds_size_mb}MB {time_val_get + 1}\n"
                        ...
                        with open(res_file, 'w', encoding='utf-8') as r_f:
                            json.dump(results, r_f, indent=4, ensure_ascii=False, sort_keys=True)
    ...
    with h_utils_path.chdir(h_utils_path.get_workspace_path() / "benchmark/lockfree_bench"):
        ...
        for ds, ds_types_map in results.items():
            for ds_type, numa_configs_map in ds_types_map.items():
                ...
                numa_config_patterns = ['same_local', 'same_remote', 'diff_setter', 'diff_getter']
                ...
                for i, pattern in enumerate(numa_config_patterns):
                    ...
                    plt.bar(x_positions + i * bar_width, dimm_times, bar_width, 
                            label=dimm_numa_config, alpha=0.8, color=colors[i])
                    plt.bar(x_positions + (i + num_patterns) * bar_width, cxl_times, bar_width, 
                            label=cxl_numa_config, alpha=0.8, color=colors[i], hatch='//')
                ...
                # plt.gca().yaxis.set_major_locator(plt.LogLocator(base=10, numticks=15))
                # plt.gca().yaxis.set_minor_locator(plt.LogLocator(base=10.0, subs=np.arange(2, 10) * 0.1, numticks=15))
                ...
                # Show the plot
                # plt.show()
                plt.savefig(f'{figs_dir}/fig_{ds}_{ds_type}_bar.pdf', 
                ...
                for pattern, marker, color in zip(numa_config_patterns, markers, colors):
                    ...
                    plt.plot(sizes, dimm_times, color=color, linestyle='-', linewidth=4, label=dimm_numa_config, marker=marker, markersize=8)
                    plt.plot(sizes, cxl_times, color=color, linestyle=':', linewidth=4, label=cxl_numa_config, marker=marker, markersize=8)
                ...
                # Show the plot
                # plt.show()
                plt.savefig(f'{figs_dir}/fig_{ds}_{ds_type}_line.pdf', 
                            bbox_inches='tight',     # Fits figure tightly
                            dpi=300,                 # Higher resolution
                            format='pdf'             # Explicitly specify format
                )
