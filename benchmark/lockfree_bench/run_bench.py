...
def install_deps():
    with h_utils_path.chdir(
        ...
        h_utils_cmd.run(
            "git clone https://github.com/jarro2783/cxxopts.git libs/cxxopt"
        )

        # install folly from github
        # h_utils_cmd.run("apt install -y libgoogle-glog-dev", True)
        h_utils_cmd.run(
            "git clone https://github.com/facebook/folly.git downloads/folly"
        )
        ...
        h_utils_cmd.run(
            "git clone https://github.com/preshing/junction.git downloads/junction"
        )
        ...
        with h_utils_path.chdir("downloads/junction"):
            ...
            with h_utils_path.chdir("build"):
                h_utils_cmd.run(
                    "cmake -DCMAKE_INSTALL_PREFIX=$(realpath ../../../libs/junction) "
                    ...
                    for numa_config_name, numa_config in numa_configs[machine].items():
                        ...
                        for ds_size_mb in ds_size_mbs:
                            ...
                            while time_val_get < 10:
                                try:
                                    stdout = h_utils_cmd.run(
                                        cmd, timeout=timeout_sec
                                    ).stdout

                                    time_pattern = re.compile(
                                        r"^\s*([\d]+)\s*ns", re.MULTILINE
                                    )
                                    time_strs = time_pattern.findall(stdout)
                                    time_vals.extend(
                                        [float(time_str) for time_str in time_strs]
                                    )
                                    time_val_get += 1
                                    ...
                                    logger.info(
                                        f"Command did not complete within "
                                        f"{e.timeout} seconds!"
                                    )
                                    second_try_times += 1
    ...
    with h_utils_path.chdir(
        ...
        for ds, ds_types_map in results.items():
            for ds_type, numa_configs_map in ds_types_map.items():
                ...
                sizes = sorted(
                    [int(size) for size in numa_configs_map[numa_configs[0]].keys()]
                )
                ...
                for i, pattern in enumerate(numa_config_patterns):
                    ...
                    dimm_times = [
                        numa_configs_map[dimm_numa_config][str(size)] / 1000000
                        for size in sizes
                    ]
                    cxl_times = [
                        numa_configs_map[cxl_numa_config][str(size)] / 1000000
                        for size in sizes
                    ]
                ...
                for pattern, marker, color in zip(
                    numa_config_patterns, markers, colors
                ):
                    dimm_numa_config = f"{pattern}_DIMM"
                    cxl_numa_config = f"{pattern}_CXL"
